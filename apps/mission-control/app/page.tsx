...
function getSessionDisplayName(session: { agentId?: string | null; key?: string | null }) {
...
export default async function Home() {
  ...
  const latestTrackedRunLabel = latestTrackedRun
    ...
    : null;
  const recentSessionActivity = data.sessionActivity;
  const latestSessionActivityLabel = recentSessionActivity?.latestUpdatedAt
    ? new Date(recentSessionActivity.latestUpdatedAt).toLocaleDateString(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })
    : null;
              ...
              <>
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-border/50 bg-card/60 p-3">
                    ...
                    <p className="mt-1 font-mono text-xl font-bold leading-tight">{recentSessionActivity.total}</p>
                  </div>
                  <div className="rounded-lg border border-border/50 bg-card/60 p-3">
                    <p className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Updated recently</p>
                    <p className="mt-1 font-mono text-xl font-bold leading-tight">{recentSessionActivity.recent}</p>
                  </div>
                </div>

                <div className="space-y-2">
                  {recentSessionActivity.sessions.map((session) => (
