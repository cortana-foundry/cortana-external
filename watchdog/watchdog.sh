...
RUNTIME_HEARTBEAT_STATE_FILE="$HOME/.openclaw/memory/heartbeat-state.json"
DEFAULT_REPO_HEARTBEAT_STATE_FILE="$CORTANA_SOURCE_REPO/memory/heartbeat-state.json"
HEARTBEAT_STATE_FILE="${HEARTBEAT_STATE_FILE:-}"
if [[ -z "$HEARTBEAT_STATE_FILE" ]]; then
  if [[ -f "$RUNTIME_HEARTBEAT_STATE_FILE" ]]; then
    HEARTBEAT_STATE_FILE="$RUNTIME_HEARTBEAT_STATE_FILE"
  else
    HEARTBEAT_STATE_FILE="$DEFAULT_REPO_HEARTBEAT_STATE_FILE"
  fi
fi
